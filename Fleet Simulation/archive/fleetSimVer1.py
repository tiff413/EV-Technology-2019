...
carData = [a, b, c, d]
labels1 = ["battPerc", "isCharge", "battSize"]
    ...
    for b in range(0, len(carDataDF)):
        for c in range(0, len(shiftsByCar[str(b)])):
            ...
            if time == startS:
                ...
                batt = carDataDF.loc[b, 'battPerc']
                battPrint = np.around(batt, decimals=1)
                print("  car " + str(b) + " leaving centre with " + str(battPrint) + " %")
...
def charge(carDataDF, carNum, chargeCen, chargeRate):
    ...
    if batt >= 100:
        ...
        # PRINT NEW BATT
        print("    new batt: "+str(100))
    else:
        carDataDF.loc[carNum, 'battPerc'] = batt

        # PRINT NEW BATT
        battPrint = np.around(batt, decimals=1)
        print("    new batt: "+str(battPrint))
