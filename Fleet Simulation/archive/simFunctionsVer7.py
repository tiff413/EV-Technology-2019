...
################################################
def readFullBattCars(carDataDF, simulationDF, time):
...
###############################
def decreaseBatt(carDataDF, shiftsByCar, time, rcCount, simulationDF, driveDataByCar, ind):
    # FOR EVERY CAR
...
def charge(carDataDF, carNum, chargeRate, simulationDF, time, chargePtDF, pricesDF):
    ...
    simulationDF = simulationDF.append({
        ...
        'cost': round((chargeRate*price)/chunks, 1) if chargeRate > 0 else 0
    }, ignore_index=True)
    # print("CHARGE")
...
############################################
def superSmartCharge(carDataDF, depot, shiftsByCar, time, chargeCapacity, simulationDF, chargePtDF, pricesDF):
    # IF THERE ARE CARS IN THE DEPOT
    if len(depot) >= 1:
        ...
        for cars in range(0, len(depot)):
            ...
            priorityRows.append([carNum, battLeft/hrsLeft.total_seconds(), battLeft])
        ...
        for row in range(0, len(leaveTimes)):
            ...
            if batt < battSize:
                ...
                if not np.isnan(pt):
                    ...
                    if chargeRate > battLeft: chargeRate = battLeft
...
# CREATE LIBRARY FOR SIMULATION NAMES TO CALL IN runSimulation
simulations = {
...
def runSimulation(startTime, runTime,
                  fleetData, driveDataDF, allShiftsDF, pricesDF,
                  algo):
    ...
    simCols = ["time","car","chargeDiff","batt","event","cost"]
    ...
    for i in range(0, runTime*chunks):
        ...
        carDataDF, simulationDF, time = readFullBattCars(carDataDF, simulationDF, time)
        carDataDF, time, rcCount, simulationDF = decreaseBatt(carDataDF, shiftsByCar, time, rcCount, simulationDF, driveDataByCar, i)
    ...
    return styleDF(sim), sim    # second dataframe, 'sim', is for animation purposes
