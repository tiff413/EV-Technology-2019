    ...
    for b in range(0, len(carDataDF)):
        for c in range(0, len(shiftsByCar[str(b)])):
            ...
            if time == startS:
                ...
                battPower = carDataDF.loc[b, 'battPower']

                kw = np.around(battPower, decimals=1)
...
def charge(carDataDF, carNum, chargeCen, chargeRate):
    ...
    battPrint = np.around(battPower, decimals=1)
    ...
    if len(chargeCen) >= 1:
        ...
        if len(chargeDF) >= 1:
            ...
            for c in range(len(chargeDF)):
                ...
                ratePrint = np.around(chargeRate, decimals=1)
                print("  car "+str(d)+" charging at "+str(ratePrint)+" kW/hr")
...
def smartCharge1(chargeCen, shiftsByCar, time, chargeCapacity, maxRate, carDataDF):
    # IF THERE ARE CARS IN THE CHARGE CENTRE
    if len(chargeCen) >= 1:
        ...
        # CHARGE CARS IN ORDER OF WHO IS LEAVING FIRST
        for h in range(0, len(leaveTimes)):
            ...
            if energyLeft >= 0 and batt < 100:
                ...
                ratePrint = np.around(chargeRate, decimals=1)
                print("  car "+str(car)+" charging at "+str(ratePrint)+" kW/hr")
...
def smartCharge2(chargeCen, shiftsByCar, time, chargeCapacity, maxRate, carDataDF):

    # IF ONE CAR IN CHARGE CENTRE, CHARGE IT AT MAX RATE
    if len(chargeCen) == 1:
        carInd = chargeCen[0]
        battPerc = carDataDF.loc[carInd, 'battPerc']
        if battPerc < 100:
            charge(carDataDF, carInd, chargeCen, maxRate)
            print("  car "+str(carInd)+" charging at "+str(maxRate)+" kW/hr")

    # IF THERE IS MORE THAN ONE CAR IN THE CHARGE CENTRE
    if len(chargeCen) > 1:
        ...
        # CHOOSE PERCENTAGE THAT CARS SHOULD CHARGE TO
        chargeToPerc = 90
        ...
        for h in range(0, len(leaveTimes)):
            car = leaveTimes.loc[h, 'car']
            timeLeft = (leaveTimes.loc[h, 'hrsLeft']).total_seconds()

            battSize = carDataDF.loc[car, 'battSize']
            battPower = carDataDF.loc[car, 'battPower']

            chargeTo = (chargeToPerc/100)*battSize
            chargeLeft = battPower - chargeTo

            prio = chargeLeft/timeLeft
            prioList.append(prio)

        chargeCap = chargeCapacity
        for j in range(0, len(prioList)):
            chargeRate = chargeCap * (prioList[j]/sum(prioList))

            if chargeRate > 7:
                chargeRate = 7

            ratePrint = np.around(chargeRate, decimals=1)
            print("  car "+str(j)+" charging at "+str(ratePrint)+" kW/hr")

            charge(carDataDF, car, chargeCen, chargeRate)

            chargeCap -= chargeRate
...
for i in range(0, runTime):
    ...
    carDataDF = smartCharge2(chargeCen, shiftsByCar, time, chargeCapacity, maxRate, carDataDF)
